...
import com.longluo.volleydemo.ssl.SslHttpStack;

import com.longluo.volleydemo.R;

/**
 * Demonstrates: StringObjectRequest usage. StringObjectRequest converts the
 * response to String. ( can be json,xml )You can then do the corresponding
...
public class StringObjectRequestActivity extends Activity {
    ...
    private void makeSampleHttpRequest() {
        ...
        // To EXPERIMENT. Enable response caching to quickly fetch the response
        // from cache, if set true.
        // Volley decides whether to cache the response or not, based on
        // response headers obtained. Some of the parameters
        // to look for are Cache-control,maxAge, Expires.
        //
        // In case of weather api, the response headers has
        // "Cache-Control: no-cache, must-revalidate" In this case, even if
        // setShouldCache() api is set true, Volley decides not to store the
        // response, because server has sent response headers as
        // "must-revalidate"
        // So storing response doesn't make sense in this api. Some of these
        // intelligences are implemented already in Volley, you need not take
        // the burden of
